                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      <select
                        value={userSortBy}
                        onChange={(e) => {
                          setUserSortBy(e.target.value as 'name' | 'email' | 'role' | 'portfolio' | 'date');
                          setCurrentPage(1);
                        }}
                        className="block w-full px-4 py-3 border border-gray-300 rounded-lg leading-5 bg-white focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-900"
                      >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-200">
                    <div className="flex items-center space-x-4 text-sm">
                      <span className="text-gray-600">
                        ...
                        })()}</span> of <span className="font-semibold text-gray-900">{users.length}</span> users
                      </span>
                    </div>
                  ...
                  const sortedUsers = [...filteredUsers].sort((a, b) => {
                    ...
                    switch (userSortBy) {
                      ...
                      case 'role':
                        ...
                        break;
                      case 'portfolio':
                        const aPortfolio = portfolios.find(p => p.userId === a._id);
                        const bPortfolio = portfolios.find(p => p.userId === b._id);
                        compareValue = (aPortfolio?.totalValue || 0) - (bPortfolio?.totalValue || 0);
                        break;
                      case 'date':
                        ...
                        <>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {paginatedUsers.map((u) => {
                              ...
                              return (
                                <div key={u._id} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100">
                                  {/* Card Header with gradient */}
                                  <div className={`h-24 bg-gradient-to-br ${u.role === 'admin' ? 'from-purple-500 to-purple-600' : 'from-blue-500 to-blue-600'} relative`}>
                                    <div className="absolute -bottom-12 left-6">
                                      <div className={`w-24 h-24 rounded-2xl ${u.role === 'admin' ? 'bg-gradient-to-br from-purple-400 to-purple-600' : 'bg-gradient-to-br from-blue-400 to-blue-600'} shadow-xl flex items-center justify-center text-white text-2xl font-bold border-4 border-white`}>
                                  ...
                                  <div className="pt-16 px-6 pb-6">
                                    ...
                                    <div className="mb-4">
                                      <div className="flex items-start justify-between mb-2">
                                        <h3 className="text-xl font-bold text-gray-900">{u.name || 'No Name'}</h3>
                                        <span className={`px-3 py-1 text-xs font-bold rounded-full ${u.role === 'admin' ? 'bg-purple-100 text-purple-700 ring-2 ring-purple-200' : 'bg-blue-100 text-blue-700 ring-2 ring-blue-200'}`}>
                                          {u.role === 'admin' ? '👑 Admin' : '👤 User'}
                                        </span>
                                      </div>
                                    ...
                                    <tr key={u._id} className="hover:bg-gray-50 transition-colors">
                                      <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                          <div className={`w-10 h-10 rounded-lg ${u.role === 'admin' ? 'bg-gradient-to-br from-purple-400 to-purple-600' : 'bg-gradient-to-br from-blue-400 to-blue-600'} flex items-center justify-center text-white text-sm font-bold`}>
                                            {getInitials(u.name, u.email)}
                                          </div>
                                          <div className="ml-4">
                                      ...
                                      <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-gray-900">{u.contact || 'N/A'}</div>
                                      </td>
                                      <td className="px-6 py-4 whitespace-nowrap">
